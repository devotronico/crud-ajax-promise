...
function fillViewModal(data){ 
    ...
    let viewtmpvar = `
        <tr>
            ...
            <td class="view__tbody__test1">${data.city}</td>
            <td class="view__tbody__test1">${data.phone}</td>
        </tr>`;
...
function appendToTable(data){
    ...
    for(let i=0; i<data.length; i++){
        ...
        let btn_view = `<td><button type="button" class="btn btn-success" id="select-id" num="${data[i].id}" data-toggle="modal" data-target="#modal__view">V</button></td>`;
        let btn_delete = `<td><button type="button" class="btn btn-danger" id="delete-id" num="${data[i].id}">D</button></td>`;
        let btn_update = `<td><button type="button" class="btn btn-warning" id="update-id" num="${data[i].id}" firstname="${data[i].firstname}" email="${data[i].email}" data-toggle="modal" data-target="#mymodal">U</button></td>`;
  ...
  export { clearTable, appendToTable, fillViewModal };